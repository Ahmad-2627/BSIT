...
```mermaid
flowchart LR
    P["📅 Planning<br/>Deciding what & how"] --> O["🧩 Organizing<br/>Arranging resources"]
    O --> L["👥 Leading<br/>Motivating people"]
    L --> C["✅ Controlling<br/>Measuring results"]
    C -.->|"feedback loop"| P
```
...
```mermaid
flowchart TD
    T["🔝 TOP MANAGERS<br/>CEO, President, Directors"] --> M["🔹 MIDDLE MANAGERS<br/>Department / Regional Managers"]
    M --> F["🔻 FIRST-LINE MANAGERS<br/>Supervisors, Team Leaders"]
    F --> E["👷 EMPLOYEES<br/>Non-managers (workers)"]
```
...
```mermaid
flowchart TD
    M[Manager] --> I[Interpersonal Roles]
    M --> N[Informational Roles]
    M --> D[Decisional Roles]
    ...
    D --> D1[Entrepreneur]
    D --> D2[Disturbance Handler]
    D --> D3[Resource Allocator]
    D --> D4[Negotiator]
```
...
```mermaid
flowchart TD
    subgraph TOP["🔝 Top Managers"]
        direction LR
        T1["Technical: LOW"]
        T2["Human: HIGH"]
        T3["Conceptual: HIGH"]
    end
    subgraph MID["🔹 Middle Managers"]
        direction LR
        M1["Technical: MEDIUM"]
        M2["Human: HIGH"]
        M3["Conceptual: MEDIUM"]
    end
    subgraph FIRST["🔻 First-Line Managers"]
        direction LR
        F1["Technical: HIGH"]
        F2["Human: HIGH"]
        F3["Conceptual: LOW"]
...
```mermaid
flowchart LR
    S["SYMBOLIC VIEW<br/>No real control"] --- R["REALITY<br/>Real but limited control"] --- O["OMNIPOTENT VIEW<br/>Full control"]
```
...
flowchart TD
    ...
    ORG --> TASK["Task Environment<br/>(Direct impact)"]
    ORG --> GEN["General Environment<br/>(PESTLE — indirect impact)"]
    ...
    GEN --> G1[Political-Legal]
    GEN --> G2[Economic]
    GEN --> G3[Sociocultural]
    GEN --> G4[Technological]
    GEN --> G5[Legal]
    GEN --> G6[Environmental/Natural]
```
...
```mermaid
flowchart LR
    A["Exporting/Importing<br/>Lowest risk & control"] --> B["Licensing"]
    B --> C["Franchising"]
    C --> D["Strategic Alliance"]
    D --> E["Joint Venture"]
    E --> F["Foreign Direct Investment<br/>Highest risk & control"]
```
...
```mermaid
flowchart TD
    ST["Strategic Goals<br/>Top Management — Long-term (3-5 yrs)"] --> TA["Tactical Goals<br/>Middle Management — Medium-term (1-2 yrs)"]
    TA --> OP["Operational Goals<br/>First-Line Management — Short-term (daily/weekly)"]
```
...
```mermaid
flowchart TD
    COR["Corporate Strategy<br/>'What businesses should we be in?'<br/>Top management & Board"] --> BUS["Business Strategy<br/>'How do we compete?'<br/>Business unit managers"]
    BUS --> FUN["Functional Strategy<br/>'How does each department support this?'<br/>Department managers"]
```
...
```mermaid
flowchart TD
    S1["1. Recognize the Problem"] --> S2["2. Identify Decision Criteria"]
    S2 --> S3["3. Allocate Weights to Criteria"]
    S3 --> S4["4. Develop Alternatives"]
    S4 --> S5["5. Evaluate Alternatives"]
    S5 --> S6["6. Select Best Alternative"]
    S6 --> S7["7. Implement Decision"]
    S7 --> S8["8. Evaluate Effectiveness"]
```
...
```mermaid
flowchart LR
    subgraph CORE["5 Core Job Characteristics"]
        C1[Skill Variety]
        C2[Task Identity]
        C3[Task Significance]
        C4[Autonomy]
        C5[Feedback]
    end
    subgraph STATES["3 Psychological States"]
        P1["Experienced<br/>Meaningfulness"]
        P2["Experienced<br/>Responsibility"]
        P3["Knowledge<br/>of Results"]
    end
    OUT["Outcomes:<br/>Motivation, Satisfaction, Performance"]
...
```mermaid
flowchart LR
    SV["Skill Variety"] --> SUM(("+"))
    TI["Task Identity"] --> SUM
    TS["Task Significance"] --> SUM
    SUM --> MULT{"× Autonomy × Feedback"}
    MULT --> MPS["Motivating Potential Score (MPS)"]
```
...
```mermaid
flowchart TD
    D[Departmentalization] --> B1["Functional<br/>By business function"]
    D --> B2["Product<br/>By product line"]
    D --> B3["Customer<br/>By customer type"]
    D --> B4["Geographic<br/>By location"]
    D --> B5["Process<br/>By workflow stage"]
```
...
```mermaid
flowchart TD
    subgraph Narrow["Narrow Span (Tall Structure)"]
        direction TB
        CEO1[CEO] --> MGRA[Manager A]
        CEO1 --> MGRB[Manager B]
        MGRA --> E1[Employee]
        MGRA --> E2[Employee]
        MGRB --> E3[Employee]
    ...
    subgraph Wide["Wide Span (Flat Structure)"]
        direction TB
        CEO2[CEO] --> E5[Employee]
        CEO2 --> E6[Employee]
        CEO2 --> E7[Employee]
        CEO2 --> E8[Employee]
        CEO2 --> E9[Employee]
...
```mermaid
flowchart TD
    A["Manager gives AUTHORITY<br/>(right to decide and act)"] --> B["Employee takes RESPONSIBILITY<br/>(obligation to perform)"]
    B --> C["Employee maintains ACCOUNTABILITY<br/>(obligation to report results)"]
    C --> D["Manager evaluates results<br/>(control function)"]
```
...
```mermaid
flowchart TD
    subgraph HighPeople["High Concern for People"]
        direction LR
        CC["(1,9) Country Club<br/>High people, low task"]
        TM["(9,9) Team Management<br/>High people, high task — IDEAL"]
    end
    subgraph LowPeople["Low Concern for People"]
        direction LR
        IMP["(1,1) Impoverished<br/>Low people, low task"]
        AC["(9,1) Authority-Compliance<br/>Low people, high task"]
    end
    MID["(5,5) Middle of the Road<br/>Moderate on both"]
    HighPeople --- MID --- LowPeople
```
...
```mermaid
flowchart LR
    R1["R1: Low ability,<br/>low willingness"] --> S1["Telling/Directing<br/>High task, low relationship"]
    R2["R2: Low ability,<br/>high willingness"] --> S2["Selling/Coaching<br/>High task, high relationship"]
    R3["R3: High ability,<br/>low willingness"] --> S3["Participating<br/>Low task, high relationship"]
    R4["R4: High ability,<br/>high willingness"] --> S4["Delegating<br/>Low task, low relationship"]
```
...
```mermaid
flowchart TD
    F["Forces for Change"] --> EXT["External Forces"]
    F --> INT["Internal Forces"]

    EXT --> E1[Technological Change]
    EXT --> E2[Economic Changes]
    EXT --> E3[Competition]
    EXT --> E4[Social & Political Changes]
    EXT --> E5[International Forces]

    INT --> I1[Declining Performance]
    INT --> I2[Changing Workforce]
    INT --> I3[New Equipment/Technology]
    INT --> I4[Employee Attitudes]
```
...
```mermaid
flowchart LR
    A["🧊 Unfreeze<br/>Break old habits"] --> B["🔄 Change<br/>Implement new ways"]
    B --> C["❄️ Refreeze<br/>Stabilize new normal"]
```
...
```mermaid
flowchart TD
    K1["1. Create Urgency"] --> K2["2. Build Guiding Coalition"]
    K2 --> K3["3. Develop Vision & Strategy"]
    K3 --> K4["4. Communicate the Vision"]
    K4 --> K5["5. Empower Broad-Based Action"]
    K5 --> K6["6. Generate Short-Term Wins"]
    K6 --> K7["7. Consolidate Gains"]
    K7 --> K8["8. Anchor Changes in Culture"]
```
